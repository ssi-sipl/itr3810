...
from fuctions import get_gps_satellites_in_view

# Load the radar API DLL
dll_path = "./path_to/ITR3800_radarAPI.dll"  # Replace with the correct path
...
# Define other function prototypes
...
handle = APIHandle_t()
result = radar_api.ITR3800_initSystem(ctypes.byref(handle), 192, 168, 31, 200)  # Replace IP address parts
if result != 0:
    raise RuntimeError(f"Failed to initialize radar API: Error code {result}")
print("Radar system initialized successfully.")

# Retrieve and display GPS satellites in view
